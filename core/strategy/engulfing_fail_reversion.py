"""Fade a failed engulfing continuation that reverts back inside the engulf bar.

After a two-bar *body* engulf (``engulfing_direction``), the box is that
engulfing bar's high/low — the engulf extremes, not the body and not a
rolling N-bar channel. A continuation tries to leave that box in the
engulf's direction; this sleeve fades the fail:

- SHORT: a bullish engulf, then a later bar trades through that engulf
  high, then close_t is back below the high (within
  ``max_bars_since_engulf``).
- LONG: a bearish engulf, then a later bar trades through that engulf
  low, then close_t is back above the low.

The clock starts at the engulf, not at the break. With the Quant grid
``[1, 2]`` that is tight on purpose:

- 1 bar after the engulf: same-bar wick-through of the continuation
  extreme, close back inside (a held close-through on that bar does not
  fire).
- 2 bars after the engulf: a prior close-through, then a later fail, or
  a same-bar wick-fail on either post-engulf bar.

Quant-locked ``require_close_inside=True``: the fail close must still sit
inside the engulf box. The only free param is ``max_bars_since_engulf``
...
class EngulfingFailReversionParams(StrategyParams):
    ...
    # Quant-locked True: fade only if close_t sits back inside the engulf box.
...
class EngulfingFailReversionStrategy(Strategy):
    ...
    def generate_signals(self, candles: pd.DataFrame) -> pd.DataFrame:
        ...
        # Body engulf on the setup bar. The box we fade is that bar's H/L.
        direction = ind.engulfing_direction(candles["open"], close)
        ...
        # A continuation break is a trade through that rail in the window —
        # wick on the fail bar, or a prior close-through after the engulf.
        ...
        # Back through the broken continuation rail. Locked True: still inside.
        short_raw = broke_up & (close < bull_high)
        long_raw = broke_down & (close > bear_low)
        if close_inside:
        ...
        if params.side is SignalSide.LONG:
            ...
            event_lag, event_level, other_level = bear_lag, bear_low, bear_high
        else:
            raw = short_raw.fillna(False)
            signal_value, side_value = -1, SignalSide.SHORT.value
            event_lag, event_level, other_level = bull_lag, bull_high, bull_low

        # One fade per engulf event, not every bar that stays inside.
        orphan = -1.0 - idx
