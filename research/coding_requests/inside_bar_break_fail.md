...
## Coding brief (CEO FINAL LOCK for coding)

- Clock: `4h/4h`
- Side: `BOTH`
- Status: CEO FINAL LOCK for coding (do not start walk-forward). Live stays off.
- Quant lock: ATR period = 20 LOCKED / not searched; ATR known before the signal bar (`atr.shift(1)`). Mother = inside-bar mother context LOCKED (`inside_bar_mother`: bar t-1 strictly inside bar t-2; any bar, not a London 07:00–11:00 session lock). `require_close_inside_mother=True` LOCKED (`mother_low < close[t] < mother_high`). SHORT = `high[t] > mother_high` AND close strictly inside mother (and not also through the low). LONG = `low[t] < mother_low` AND close strictly inside mother (and not also through the high). Free search (1 only): `min_mother_atr` `[0.8, 1.0, 1.2]` (endpoints 0.8 and 1.2 plus 0.2-step interior, matching how sibling single-float grids list discrete search points). Size: `(mother_high - mother_low) >= min_mother_atr * ATR20`. Fill at `t+1` open. Family id `inside_bar_break_fail` only. Do not rename free params. Do not clone `ib_fail_reversion`. Do not recode `lvn_fill_reject`.
