# Proposal: `stochastic_cmf_divergence_fade`
...
1. `core/strategy/stochastic_cmf_divergence_fade.py` — `Strategy` subclass, `name = "stochastic_cmf_divergence_fade"`.
2. Signals may use bars `<= t` only; the engine fills at `t+1` open.
3. Tests: schema, no lookahead (truncation + future shock), at least one entry.
4. Do not copy a rejected family and rename it.
5. After `list_strategies()` contains this name, call `firm.cursor_coding.mark_done("stochastic_cmf_divergence_fade")`.
