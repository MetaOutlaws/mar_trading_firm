...
RESEARCH_HYPOTHESES: list[dict[str, Any]] = [
    ...
    {
        "id": "close_location_persistence@4h/4h",
        "family": "close_location_persistence",
        "name": "close_location_persistence 4h/4h BOTH",
        "clock": "4h/4h",
        "side": "BOTH",
        "rank": 9,
        "coded": True,
        ...
        "disposition": "new_family",
        "justification": (
            "Inbox walk ninth. Mean CLV=(close-low)/(high-low) over lookback "
            "8: LONG when mean >= 0.75 and close is not a 20-bar high; SHORT "
            "when mean <= 0.25 and close is not a 20-bar low. BOTH sides "
            "honest. Not body_efficiency_follow, wick_rejection_reversal, "
            "turnover, session/VWAP, or week-open."
        ),
        "param_change": {"clock": "4h/4h"},
        "needs_feed": False,
    },
    {
        "id": "open_in_prior_range_fail@4h/4h",
        "family": "open_in_prior_range_fail",
        "name": "open_in_prior_range_fail 4h/4h BOTH",
        "clock": "4h/4h",
        "side": "BOTH",
        "rank": 10,
        "coded": True,
        ...
        "disposition": "new_family",
        "justification": (
            "Inbox walk tenth. If this 4h opens outside the prior 4h high-low "
            "and closes back inside, fade toward the prior-bar mid. BOTH "
            "sides honest. Not utc_open_fail_reversion (UTC first-4h box) "
            "and not ny_cash_open_drive."
        ),
        "param_change": {"clock": "4h/4h"},
        "needs_feed": False,
    },
    {
        "id": "equal_high_low_restest_fade@4h/4h",
        "family": "equal_high_low_restest_fade",
        "name": "equal_high_low_restest_fade 4h/4h BOTH",
        "clock": "4h/4h",
        "side": "BOTH",
        "rank": 11,
        "coded": True,
        ...
        "disposition": "new_family",
        "justification": (
            "Inbox walk eleventh. Rolling equal high/low restest fail: two "
            "prior highs (lows) match within an ATR tolerance, then this bar "
    ...
    {
        "id": "failed_range_break_reversion@4h/4h",
        "family": "failed_range_break_reversion",
        "name": "failed_range_break_reversion 4h/4h BOTH",
        "clock": "4h/4h",
        "side": "BOTH",
        "rank": 21,
        "coded": True,
        ...
        "disposition": "new_family",
        "justification": (
            "Brian Inbox-approved for coding only. Fade a 4h close back "
            "inside a prior N-bar range after a close-through that fails "
            "to hold. Quant-locked grid: require_close_inside=True, "
            "lookback [16, 20], max_bars_since_break [2, 3], no volume. "
            "BOTH sides honest. Walk-forward is not started from this "
            "coding PR."
        ),
        "param_change": {"clock": "4h/4h"},
        "needs_feed": False,
    ...
    {
        "id": "three_black_crows@4h/4h",
        "family": "three_black_crows",
        "name": "three_black_crows 4h/4h SHORT",
        "clock": "4h/4h",
        "side": "SHORT",
        "rank": 35,
        "coded": True,
        ...
        "justification": (
            ...
            "max_upper_wick_frac [0.15, 0.25]. SHORT side honest. "
            "Walk-forward is not started from this coding PR."
        ),
        "param_change": {"clock": "4h/4h"},
        "needs_feed": False,
    ...
    {
        "id": "prior_poc_reclaim_fade@4h/4h",
        "family": "prior_poc_reclaim_fade",
        "name": "prior_poc_reclaim_fade 4h/4h BOTH",
        "clock": "4h/4h",
        "side": "BOTH",
        "rank": 43,
        "coded": True,
        ...
        "disposition": "new_family",
        "justification": (
            "Brian Inbox-approved for coding only. Tag the prior "
            "completed UTC-day volume-profile POC then reclaim "
            "toward value through POC. Quant-locked grid: POC from "
            "the prior COMPLETED UTC day only (00:00–24:00 "
            "yesterday; forming day excluded), 20 equal-width "
            "price bins, volume histogram, POC = highest-volume "
            "bin midpoint, ATR period=20 known before the signal "
            "bar (atr.shift(1)), no k-stretch. SHORT=high[t] >= "
            "POC - touch_tol_atr*ATR AND close[t] < POC / "
            "LONG=low[t] <= POC + touch_tol_atr*ATR AND close[t] "
            "> POC. Free search (1 only): touch_tol_atr [0.0, "
            "0.10]. BOTH sides honest. Walk-forward is not "
            "started from this coding PR. Live stays off. Not "
            "prior_day_extreme_reject (118 H/L), not "
            "sma20_stretch_fade (141), not keltner_channel_fade "
            "(Job 144 0/12 — do not revive), not keltner_break, "
            "not bb_medium_bw_upper_reject, not "
            ...
            "week_open_reclaim / orb_fail_reversion."
        ),
        "param_change": {"clock": "4h/4h"},
        "needs_feed": False,
    },
    ...
    },
    {
        ...
        "justification": (
            ...
            "VAH - touch_tol_atr*ATR AND VAL < close < VAH / "
            "LONG=low[t] <= VAL + touch_tol_atr*ATR AND "
            "VAL < close < VAH. Free search (3 only): lookback "
            "[20, 48], touch_tol_atr [0.0, 0.10], va_frac "
            "[0.68, 0.70]. "
    ...
    },
    {
        ...
        "disposition": "new_family",
        "justification": (
            "AUTHORITATIVE Munha/Garwe stamp for coding only "
            "(Brian YES already live). Fade a 4h sized thrust "
            "that breaks the immediate prior high or prior low "
            "then closes back inside that prior bar. SHORT "
            "priority: failed upside thrust is the primary "
            "edge; LONG is the downside mirror. Quant-locked "
            "grid: prior = bar t-1, thrust/fail = same bar t, "
            "thrust = (high-low) >= min_thrust_atr * ATR20 AND "
            "exclusive high/low break of prior (wick counts), "
            "require_close_inside=True (close back inside "
            "prior), ATR period=20 known before the signal bar "
            "(atr.shift(1)), fill t+1 open. SHORT=high[t] > "
            "prior_high AND prior_low < close[t] < prior_high / "
            "LONG=low[t] < prior_low AND prior_low < close[t] "
            "< prior_high. Free search (1 only): min_thrust_atr "
            "[1.0, 1.5] (endpoints only, same sibling "
            "float-grid as min_outside_atr / min_mother_atr). "
            "BOTH sides honest (registered BOTH, not a "
            "SHORT-only clone). Walk-forward is not started "
            "from this coding PR. Live stays off. Desk walks "
            "Job ~151 ONE after merge. Not "
            "failed_range_break_reversion (119 — rolling "
            "Donchian), not nr7 / ib / orb fail, not "
            "outside_bar_fail_reversion, not "
            "inside_bar_break_fail (family F), not "
            "expansion_fail_fade (131), not "
            "candle_reject_reversal, not VP cluster / "
            "lvn_fill_reject (family E). Hold H&S/asia/wyckoff "
            "alone. No DOGE/DOT/NEAR/LDO universe adds."
        ),
        "param_change": {"clock": "4h/4h"},
        "needs_feed": False,
    },
    ...
    },
    {
        ...
        "disposition": "new_family",
        "justification": (
            "CEO YES / Brian AUTHORITATIVE FINAL stamp for "
            "coding only (Job ~154). Fade a 4h three-push "
            "exhaustion that fails to extend (SHORT = 3 sized "
            "higher highs then no 4th HH; LONG = 3 sized lower "
            "lows then no 4th LL). Quant-locked grid: n_pushes=3 "
            "(exact run on t-3..t-1; t-4 was not a sized push), "
            "ATR period=20 known before the signal bar "
            "(atr.shift(1)), fail-to-extend on bar t "
            "(high[t] <= high[t-1] SHORT / low[t] >= low[t-1] "
            "LONG), fill t+1 open, no volume/session gate. "
            "SHORT=three sized HH then fail / LONG=three sized "
            "LL then fail. BOTH sides honest, SHORT priority on "
            "two-sided bars. Free search (1 only): min_push_atr "
            "[0.15, 0.35] (endpoints only, same sibling "
            "float-grid as min_outside_atr / min_mother_atr / "
            "min_break_atr). Walk-forward is not started from "
            "this coding PR — register for Desk ONE walk after "
            "merge (Mukanya starts walks). Live stays off. Not "
            "consecutive_bar_exhaustion (N closes), not "
            "failed_higher_high, not "
            "swing_break_fail_reversion (family I / Job 153), "
            "not swing_failure_reversal, not key_reversal_bar "
            "(family H / Job 152), not "
