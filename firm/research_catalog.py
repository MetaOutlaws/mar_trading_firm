...
RESEARCH_HYPOTHESES: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "disposition": "new_family",
        "justification": (
            "Munha stamp for coding only (Brian YES already "
            "live). Fade a 4h thrust break of the immediate "
            "prior bar that fails (next close back inside that "
            "prior range). SHORT priority: failed upside "
            "thrust-break is the primary edge; LONG is the "
            "downside mirror. Quant-locked grid: prior = bar "
            "t-2, thrust/break = close-through of that prior on "
            "bar t-1 sized by H-L vs ATR20, require_close_"
            "inside=True (next close back inside prior), ATR "
            "period=20 known before the signal bar "
            "(atr.shift(1)), fill t+1 open, next-bar only. "
            "SHORT=close[t-1] > prior_high AND prior_low < "
            "close[t] < prior_high / LONG=close[t-1] < "
            "prior_low AND prior_low < close[t] < prior_high. "
            "Free search (1 only): min_thrust_atr [1.0, 1.5] "
            "(endpoints only, same sibling float-grid as "
            "min_outside_atr / min_mother_atr). BOTH sides "
            "honest (registered BOTH, not a SHORT-only clone). "
            "Walk-forward is not started from this coding PR. "
            "Live stays off. Not failed_range_break_reversion "
            "(119 — rolling Donchian), not nr7 / ib / orb fail, "
            "not outside_bar_fail_reversion, not "
            "inside_bar_break_fail (family F), not "
            "expansion_fail_fade (131), not "
            "lvn_fill_reject (family E)."
        ),
        "param_change": {"clock": "4h/4h"},
        "needs_feed": False,
    },
