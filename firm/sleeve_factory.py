...
CANDIDATE_SPECS: list[SleeveSpec] = [
    ...
    SleeveSpec(
        ...
        needs_feed=False,
        novel_reason=(
            "Rolling volume-profile value-area extreme reject as "
            "one 4h BOTH family. VA is the rolling volume "
            "profile on the walk clock from the prior lookback "
            "bars (signal bar excluded). Same locked histogram "
            "as prior_poc_reclaim_fade / hvn_mean_revert: 20 "
            "equal-width price bins; volume spread uniformly "
            "across overlapping bins. VA grows one bin at a "
            "time from POC until va_frac of volume; VAH/VAL = "
            "outer edges. SHORT: high[t] >= VAH - "
            "touch_tol_atr*ATR AND VAL < close[t] < VAH. LONG: "
            "low[t] <= VAL + touch_tol_atr*ATR AND VAL < "
            "close[t] < VAH. require_close_inside_va locked "
            "True. ATR period locked 20, known before the "
            "signal bar (atr.shift(1)). Fill t+1 open. Free "
            "search (3 only): lookback [20, 48], touch_tol_atr "
            "[0.0, 0.10], va_frac [0.68, 0.70]. Not "
            "prior_poc_reclaim_fade "
            "(Job 145 — single prior-day POC). Not "
            "hvn_mean_revert (Job 146 — nearest-of-top-N). Not "
            ...
            "asia / inventory fades. Do not recode spent "
            "families 118–146. Do not modify sibling geometry."
        ),
        summary=(
            "Fade a 4h tag of the rolling volume-profile "
            "value-area extreme that closes back inside the VA "
            "(SHORT = VAH reject, LONG = VAL reject) as one 4h "
            "BOTH family."
        ),
        justification=(
            "A locked rolling walk-clock value-area extreme "
            "fade, with ATR20 known before the signal bar and "
            "searched lookback + touch_tol_atr + va_frac only, is "
            "not a prior-day POC / HVN reclaim, not a VWAP "
            "stretch reject, and not a session-volume-profile "
            "skip-list family."
        ),
    ),
    SleeveSpec(
    ...
    SleeveSpec(
        ...
        novel_reason=(
            ...
            "(SHORT priority). AUTHORITATIVE Munha/Garwe: "
            "thrust is range >= k·ATR AND breaks prior high or "
            "prior low (wick counts; exclusive one-sided); fail "
            "is close back inside the prior bar. Same-bar: prior "
            "is t-1, thrust/fail is t. SHORT: high[t] > "
            "prior_high AND not low[t] < prior_low AND prior_low "
            "< close[t] < prior_high. LONG: low[t] < prior_low "
            "AND not high[t] > prior_high AND prior_low < "
            "close[t] < prior_high. ATR period locked 20, known "
            "before the signal bar (atr.shift(1)). Fill t+1 "
            "open. Free search (1 only): min_thrust_atr "
            "[1.0, 1.5] (endpoints only, same sibling float-grid "
            "as min_outside_atr / min_mother_atr). No volume "
            "gate. Not failed_range_break_reversion (119 — "
            "rolling N-bar Donchian). Not nr7_fail_reversion "
            "(123). Not ib_fail_reversion (124). Not "
            "orb_fail_reversion. Not outside_bar_fail_reversion "
            "(both rails then next close-inside vs mid). Not "
            "inside_bar_break_fail (family F — IB mother "
            "wick-fail). Not expansion_fail_fade (131). Not "
            "candle_reject_reversal. Not "
            "range_compression_volume_thrust (102 — follow). Not "
            "atr_open_flush_fade (138). Not "
            "engulfing_fail_reversion (126). Not VP cluster / "
            "lvn_fill_reject (family E). Do not recode family F. "
            "Do not recode spent families 118–150. Do not "
            "modify sibling geometry. Hold H&S/asia/wyckoff "
            "alone. No DOGE/DOT/NEAR/LDO universe adds."
        ),
        summary=(
            "Fade a 4h sized thrust that breaks the prior high "
            "or prior low then closes back inside that prior "
            "bar (SHORT = failed upside thrust, LONG = failed "
            "downside thrust) as one 4h BOTH family with SHORT "
            "priority."
        ),
        justification=(
            "A locked ATR20 same-bar high/low break of the "
            "immediate prior bar, sized by min_thrust_atr, then "
            "close back inside that prior bar, is not a rolling "
            "Donchian fail, not an NR7 / London-IB / ORB fail, "
            "not an outside-bar both-rail next-bar mid-split, "
            "not family-F IB-mother wick-fail, not expansion "
    ...
    SleeveSpec(
        ...
        novel_reason=(
            ...
            "(SHORT priority). Garwe geometry SUPERSEDES looser "
            "Munha fail-to-extend OR. Pushes live on t-3, t-2, "
            "t-1. SHORT: high[t-3] < high[t-2] < high[t-1] AND "
            "both successive advances >= min_push_atr*ATR20 AND "
            "high[t] <= high[t-1] AND close[t] < open[t] AND "
            "close[t] < close[t-1]. LONG: inverse (three falling "
            "lows then fail new low + strong close). Exactly 3 "
            "prior pushes. Size gate >=. Two-sided fail prints "
            "SHORT, not LONG. ATR period locked 20, known before "
            "the signal bar (atr.shift(1)). Fill t+1 open. No "
            "volume/session/VP gate. Free search (1 only): "
            "min_push_atr [0.15, 0.35] (endpoints only; do NOT "
            "use [0.3, 0.6]). Not consecutive_bar_exhaustion. "
            ...
            "thrust_bar_fail_reversion (family G / Job 151). Not "
            "inside_bar_break_fail (family F). Not "
            "outside_bar_reversal / outside_bar_fail_reversion. "
        ...
        summary=(
            "Fade a 4h three-push exhaustion that fails a 4th "
            "extreme and prints a strong reverse close "
            "(SHORT = 3 sized HH then fail+bear close; "
            "LONG = 3 sized LL then fail+bull close) as one 4h "
            "BOTH family with SHORT priority."
        ),
        justification=(
            "A locked ATR20 exact-three prior bar-extreme push "
            "(n_pushes=3) sized by min_push_atr (>=) then "
            "fail-new-extreme AND strong reverse close, with "
            "searched min_push_atr [0.15, 0.35] only (not "
            "[0.3, 0.6]), is not a consecutive-close fade, not a "
            "confirmed-pivot failed HH, not a lookback-N swing "
            "wick-fail, not a t-1 key-reversal break, not a "
            "thrust/IB/outside clone, not a dead VP cluster, and "
            "not Job 133 rectangle / other finished 0/12 clones. "
            "Garwe strong-close conjuncts supersede looser Munha "
            "fail-to-extend-OR."
        ),
    ),
]
