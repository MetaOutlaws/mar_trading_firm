...
def test_coded_candidate_novels_are_registered_not_ready() -> None:
    ...
    assert "prior_day_extreme_reject" not in names
    ...
    assert "ib_fail_reversion" not in names
    assert "converging_wedge_break" not in names
    ...
    assert "failed_break_reclaim" not in names
    assert "expansion_fail_fade" not in names
    ...
    assert "sma20_stretch_fade" not in names
    assert "outside_bar_fail_reversion" not in names
    assert "displacement_gap_follow" not in names
    assert "keltner_channel_fade" not in names
