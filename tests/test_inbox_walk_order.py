...
def test_inbox_walk_order_is_pr14_three_then_chart_pattern_three() -> None:
    """Leftover walk order: PR-12/14 eleven, then double-bottom, double-top, triangle."""
    ...
    assert first[:14] == list(INBOX_WALK_ORDER)
...
def test_inbox_names_stay_both_max_two_free_params_no_skip_bull() -> None:
    """Inbox walk-order families: fourteen names, BOTH honest, at most two free params."""
    assert len(INBOX_BOTH_FAMILIES) == 14
