...
def test_infer_family_from_title_and_payload() -> None:
    ...
    assert infer_family({}, "Next: displacement_gap_follow") == "displacement_gap_follow"
    assert infer_family({"family": "displacement_gap_follow"}, "ATR channel breakout") == (
...
def test_research_plan_has_ranked_backlog() -> None:
    ...
    assert "displacement_gap_follow" in coded
    ...
    assert "displacement_gap_follow" not in novel_ready
...
def test_file_novel_inbox_puts_full_brief_on_each_family(firm_db, tmp_path, monkeypatch) -> None:
    ...
    assert "displacement_gap_follow" not in families
