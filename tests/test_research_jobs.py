...
def test_infer_family_from_title_and_payload() -> None:
    ...
    assert infer_family({}, "Next: atr_open_flush_fade") == "atr_open_flush_fade"
    ...
    assert infer_family({}, "Next: sma20_stretch_fade") == "sma20_stretch_fade"
    assert infer_family({"family": "sma20_stretch_fade"}, "ATR channel breakout") == (
        "sma20_stretch_fade"
    )
    assert infer_family({}, "Next: outside_bar_fail_reversion") == (
    ...
    assert infer_family({}, "Next: prior_poc_reclaim_fade") == "prior_poc_reclaim_fade"
    assert infer_family({"family": "prior_poc_reclaim_fade"}, "ATR channel breakout") == (
        "prior_poc_reclaim_fade"
    )
    assert infer_family({}, "Next: hvn_mean_revert") == "hvn_mean_revert"
    assert infer_family({"family": "hvn_mean_revert"}, "ATR channel breakout") == (
        "hvn_mean_revert"
    )
    assert infer_family({}, "Next: prior_day_vwap_reject") == "prior_day_vwap_reject"
    assert infer_family({"family": "prior_day_vwap_reject"}, "ATR channel breakout") == (
    ...
    assert infer_family({}, "Next: lvn_fill_reject") == "lvn_fill_reject"
    assert infer_family({"family": "lvn_fill_reject"}, "ATR channel breakout") == (
        "lvn_fill_reject"
    )
    assert infer_family({}, "Code lvn fill reject 4h") == "lvn_fill_reject"
    assert infer_family({}, "Next: inside_bar_break_fail") == "inside_bar_break_fail"
    assert infer_family({"family": "inside_bar_break_fail"}, "ATR channel breakout") == (
        "inside_bar_break_fail"
    )
    assert infer_family({}, "Code inside bar break fail 4h") == "inside_bar_break_fail"
    assert infer_family({}, "Next: thrust_bar_fail_reversion") == "thrust_bar_fail_reversion"
    assert infer_family({"family": "thrust_bar_fail_reversion"}, "ATR channel breakout") == (
        "thrust_bar_fail_reversion"
    )
    assert infer_family({}, "Code thrust bar fail reversion 4h") == (
        "thrust_bar_fail_reversion"
    )
    assert infer_family({}, "Next: key_reversal_bar") == "key_reversal_bar"
    assert infer_family({"family": "key_reversal_bar"}, "ATR channel breakout") == (
        "key_reversal_bar"
    )
    assert infer_family({}, "Code key reversal bar 4h") == "key_reversal_bar"
    assert infer_family({}, "Next: swing_break_fail_reversion") == "swing_break_fail_reversion"
    assert infer_family({"family": "swing_break_fail_reversion"}, "ATR channel breakout") == (
...
def test_research_plan_has_ranked_backlog() -> None:
    ...
    assert "prior_day_extreme_reject" in coded
    assert "failed_range_break_reversion" in coded
    assert "asia_range_london_reject" in coded
    ...
    assert "lvn_fill_reject" in coded
    assert "inside_bar_break_fail" in coded
    ...
    assert "prior_day_extreme_reject" not in novel_ready
    assert "failed_range_break_reversion" not in novel_ready
    assert "asia_range_london_reject" not in novel_ready
    ...
    assert "lvn_fill_reject" not in novel_ready
    assert "inside_bar_break_fail" not in novel_ready
...
def test_file_novel_inbox_puts_full_brief_on_each_family(firm_db, tmp_path, monkeypatch) -> None:
    ...
    assert "prior_day_extreme_reject" not in families
    assert "failed_range_break_reversion" not in families
    assert "asia_range_london_reject" not in families
    ...
    assert "lvn_fill_reject" not in families
    assert "inside_bar_break_fail" not in families
